WITH web_v1 AS (
    SELECT
        ...
        SUM(ws_sales_price) AS cume_sales
    FROM web_sales
...
store_v1 AS (
    SELECT
        ...
        SUM(ss_sales_price) AS cume_sales
    FROM store_sales
...
FROM (
    ...
    FROM (
        SELECT CASE WHEN web.item_sk IS NOT NULL THEN web.item_sk ELSE store.item_sk END AS item_sk,
