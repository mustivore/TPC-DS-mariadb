WITH ssr AS (
    ...
    WHERE
        d_date BETWEEN CAST('2002-08-14' AS DATE) AND DATE_ADD(CAST('2002-08-14' AS DATE), INTERVAL 30 DAY)
...
FROM
    (SELECT 'store channel' AS channel, CONCAT('store', store_id) AS id, sales, returns, profit FROM ssr
     UNION ALL
     SELECT 'catalog channel' AS channel, CONCAT('catalog_page', catalog_page_id) AS id, sales, returns, profit FROM csr
     UNION ALL
     SELECT 'web channel' AS channel, CONCAT('web_site', web_site_id) AS id, sales, returns, profit FROM wsr) x
